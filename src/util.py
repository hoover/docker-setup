...
def get_collections_data(new_collection=None, for_dev=False):
    if not os.path.isfile(docker_file_name):
        return [], start_snoop_port, default_pg_port + 1

    collections = []
    last_snoop_port = start_snoop_port - 1
    last_pg_port = default_pg_port
    ...
    with open(docker_file_name) as collections_file:
        ...
        for service, settings in collections_settings['services'].items():
            ...
            if service.startswith('snoop-pg--') and for_dev:
                port = int(settings.get('ports', '%d' % default_pg_port)[0].split(sep=':')[0])
                if port > last_pg_port:
                    last_pg_port = port

    return collections, last_snoop_port + 1, last_pg_port + 1
